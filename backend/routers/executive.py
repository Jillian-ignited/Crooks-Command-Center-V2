    ...
    for hashtag, count in hashtag_counts.most_common(15):
        if count >= 3:
            trending_topics.append({
                ...
                "frequency": count,
                "type": "hashtag",
                "trend_strength": "high" if count >= 10 else "medium" if count >= 5 else "emerging"
            })
    
    # Top keywords
    for keyword, count in keyword_counts.most_common(10):
        if count >= 5 and len(keyword) > 4:
            trending_topics.append({
                "term": keyword,
                "frequency": count,
                "type": "keyword",
                "trend_strength": "high" if count >= 15 else "medium" if count >= 8 else "emerging"
            })
    
    return sorted(trending_topics, key=lambda x: x['frequency'], reverse=True)[:20]

def generate_real_strategic_recommendations(
    shopify_data: Dict,
    ...
    if shopify_data.get('status') == 'no_data':
        missing_data.append('Shopify sales data')
    if competitive_data.get('status') == 'no_data':
        missing_data.append('Competitive intelligence data')
    if not trending_topics:
        missing_data.append('Social media trending data')
    
    if missing_data:
        recommendations.append({
            "title": "Complete Data Integration Setup",
            "description": f"Missing critical data sources: {', '.join(missing_data)}. Full strategic analysis requires all data inputs.",
            "priority": "critical",
            "context": "Strategic recommendations depend on comprehensive data analysis across e-commerce, competitive intelligence, and social media performance.",
            "expected_impact": "Enables complete platform functionality and trustworthy insights",
            "time_to_implement": "1-2 days",
            "success_metrics": ["All data sources connected", "Complete competitive analysis available"],
            "specific_actions": [
                f"Upload {data_type} to enable full analysis" for data_type in missing_data
            ]
        })
    
    # Revenue optimization (if Shopify data available)
...
# Keep existing upload endpoints but add real data validation...
